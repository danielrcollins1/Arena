...
/******************************************************************************
 * net.superdan.athena.Monster (hostile or benign creature).
 *
...
public class Monster {
	...
	*/
	public Monster (Monster src) {
		...
		host = src.host;
		charmer = src.charmer;
		specialList = new ArrayList<>(src.specialList);
		conditionList = new ArrayList<>(src.conditionList);
	...
	public float getHitDiceAsFloat() {
		return hitDiceAsFloat;
	}
	...
	public int getMV() {
		return getMoveInches();
	}

	public int getEHD() {
		return getEquivalentHitDice();
	}
	...
	// Null methods for net.superdan.athena.Character inheritance
	...
	*/
	boolean isImmuneToEnergy (EnergyType energy) {
		return switch (energy) {
			case Fire -> hasSpecial(SpecialType.FireImmunity);
			case Cold -> hasSpecial(SpecialType.ColdImmunity);
			case Acid -> hasSpecial(SpecialType.AcidImmunity);
			case Lightning -> hasSpecial(SpecialType.LightningImmunity);
			default -> false;
		};
	} 
