...
public class MonsterTables implements Iterable<List<Monster>> {
	...
	public static MonsterTables getInstance() {
		if (instance == null) {
			try {
				...
				System.out.println("Failed to read the net.superdan.athena.Monster Level Matrix file.");
			}
		}
		return instance;
	}
	...
	*/
	private void readMonsterLevelMatrix () throws IOException {
		...
		monsterLevelMatrix = new ArrayList<>();
	...
	*/
	private void compileMonsterTables () {
		...
		monsterTables = new ArrayList<>(maxLevel);
		for (int i = 0; i < maxLevel; i++) {
			monsterTables.add(new ArrayList<>());
		}
	...
	*/
	private int randomMonsterLevel (int dungeonLevel) {
		...
		for (int level = (matrixRow != null ? matrixRow.size() : 0) - 1; level > 0; level--) {
